...
def on_collaborate(code, ask, render_backend, use_vstore, use_cache,
    ...
    yield (
        ...
        running_html("正在分析原始 shader 并改写（分析 → 生成 → 编译校验）…"),
        "", gr.update(),
    )
    opts = _opts_from_ui(render_backend, use_vstore, use_cache,
                         critique, max_fix, top_k)
...
_PALETTE_CHOICES = ["", "neon", "warm sunset", "monochrome", "cool blue",
                    "pastel", "vibrant"]
_COMPLEXITY_CHOICES = ["minimal", "simple", "moderate", "complex"]
    ...
    with gr.Blocks(
        ...
        with gr.Tab("① Analyzer · 解读现成 shader"):
            ...
            btn_analyze.click(
                ...
                outputs=[img_out, report_md, report_json,
        ...
        with gr.Tab("② Generator · 按中文需求写新 shader"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        dynamic_in = gr.Checkbox(label="动态（带 iTime）", value=True)
            ...
            btn_gen.click(
                ...
                inputs=[prompt_in, palette_in, complexity_in, dynamic_in]
                       + common_inputs,
                outputs=[gen_img_out, gen_code_out, gen_explain_md,
        ...
        # Tab 3: Collaboration
        # ============================================================
        with gr.Tab("③ Collaboration · 先分析后改写"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    collab_ask_in = gr.Textbox(
                        label="改写指令（中文）",
                        placeholder=(
                            "例：保留 raymarching 算法主干，但换成霓虹紫主题，"
                            "并在中心加一个柔和光斑"
                        ),
                        lines=3,
                    )
                    btn_collab = gr.Button(
                        "▶ 先分析后改写", variant="primary",
                    )
                with gr.Column(scale=1):
                    with gr.Row():
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    with gr.Accordion("分析报告（Analyzer 中间产物）", open=True):
